...
Does "C because A" convey no meaning, or just very little meaning?
...
Why does "C because A" convey no meaning about the state of the world?

Well, there are 4 possible world states: (!A, !B), (!A, B), (A, !B), and (A, B).

So let's look at the counterfactual probability of !C given !A with each of these world states set as the actual world.
...
cfprob3givenNotA = function(a, b, c) {
  output = paste("{A:",
                 ifelse(a, "true", "false"),
                 ", B:",
                 ifelse(b, "true", "false"),
  ...
  rs = webppl(program_file = "rsa.base.booleans/lk3/autoexpanded.wppl",
              ...
              model_var = paste(
                ...
                ") }"),
              packages = "./node_modules/jsUtils")
  rs$actual = print_world(a, b, c)
  return(sum(rs$prob[!rs$C]))
}
cfprob3givenNotA(F, F, F)
cfprob3givenNotA(F, T, T)
cfprob3givenNotA(T, F, T)
cfprob3givenNotA(T, T, F)
```
...
```{r}
# ## a bunch of alternative utterances: that's in transform.py, etc.
# ## all utterances equally probable: also set in transform.py, etc.
...
#                                    "/autoexpanded.wppl", sep=""),
#               inference_opts = list(method="enumerate"),
#               model_var = "function() {return speaker('all_alternatives');}",
#               packages = "./node_modules/jsUtils")
#   return(
...
#          story = factor(story))
# lk.model.with.expl %>% ggplot(., aes(x=model.expl.rating, y=rating)) +
#    geom_point()
```
