...
bacon_program = webppl(pdgram_file = "bacon/bacon_program.wppl",
       inference_opts = list(method = "enumerate"),
...
bacon_program_expanded = webppl(
  program_file = "bacon/bacon-autoexpanded.wppl",
...
plotmat(M, pos=c(2,2,2,2), name=names, 
        ...
        box.lcol = ifelse(names%in%c("0.25", "0.1", ""), "white", "black"),
        box.type="circle",
...
lk = function(num) {
  rs = webppl(
    program_file=paste("lk", num, "/lk", num, "-autoexpanded.wppl", sep=""),
    inference_opts = list(method="enumerate"),
    model_var = paste("function() {return counterfactual(0.53, ", cf.premises[num], "); }", sep=""),
    packages = "./node_modules/jsUtils")
  marginals = rs %>%
    gather("variable", "value", -prob) %>%
    group_by(variable) %>%
    summarise(prob = sum(prob[value==T]))
  p = ggplot(marginals, aes(x=variable, y=prob, group=1)) +
    geom_point() +
    geom_line() +
    ylim(0,1.001)
  return(p)
}
multiplot(lk(1), lk(2), lk(3), lk(4), lk(5), lk(6), cols=3)
```
