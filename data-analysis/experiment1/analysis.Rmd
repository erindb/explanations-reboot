...
mcmc = read.csv("../../models/08-webppl-only/s2_onelink_noentailments_uevenmore_MCMC40K40K200K.csv") %>%
  rename(mcmc = rating) %>%
...
compareMCMC %>% ggplot(., aes(x=enumeration, y=mcmc)) +
  geom_point()
with(compareMCMC, cor(enumeration, mcmc))
```

It matches pretty well. I'm going to go with this and use MCMC on more complicated models.
