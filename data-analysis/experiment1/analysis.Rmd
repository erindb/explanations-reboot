...
Apparently, we replicated our previous experiment perfectly. r=`r with(compare_subset, cor(lk_only, full_version))`

Does that even seem plausible given the split half correlation?

```{r}
participants = unique(df$workerid)
half = rbinom(90, 1, 0.5)
h1 = participants[half==1]
h2 = participants[half==0]
d1 = df %>% filter(workerid %in% h1) %>%
  group_by(story, utterance,
           explanandumVariable,
           explanansValue) %>%
  summarise(mean1 = mean(response))
d2 = df %>% filter(workerid %in% h2) %>%
  group_by(story, utterance) %>%
